...
function forAll(Folder, group)
    ...
    for i= 1:size(nameFolds)
       kh_SAM_TimeInt( strcat(Folder, filesep, nameFolds{i,1}), nameFolds{i}, .4, .6, group)
    end
end
...
% sum_vs_IntOfIn = abs(sum(vs_IntOfIn')); % cancelled sich raus
sum_vs_IntOfIn = sum(abs(vs_IntOfIn')); % dies ist richtig! 
...
str_timeInt= strcat('ERF_noise_abs', '_', num2str(TimeBeg), '-', num2str(TimeEnd), 's_', SubjectName);
...
function kh_reduceERF2Brain (SubjectPath, SubjectName, TimeBeg, TimeEnd)
    ...
    FileNameOld = strcat('ERF_noise_abs_', num2str(TimeBeg), '-', num2str(TimeEnd), 's', '_', SubjectName, '+tlrc');
...
function kh_z_transform (SubjectPath, SubjectName, TimeBeg, TimeEnd)
    ...
    FileName = strcat('brain01ERF_noise_abs_', num2str(TimeBeg), '-', num2str(TimeEnd), 's', '_', SubjectName, '+tlrc');   
...
eval (['!3dcopy ', PathERF, ' ',strcat('br_z_transf_brain01ERF_noise_abs', num2str(TimeBeg), '-', num2str(TimeEnd), 's_', SubjectName, 'MNI.nii')])
